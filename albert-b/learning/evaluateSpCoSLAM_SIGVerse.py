...
def ARI(Ct,It):  #Ct = []  推定された場所概念のindex  
  ...
  #正解データを読み込みCT
  for line in open( datasetfolder + correct_Ct, 'r'):
    itemList = line[:].split(',')
  ...
  #正解データを読み込み
  for line in open(datasetfolder + correct_It, 'r'):
    itemList = line[:].split(',')
...
def ReadCorrectSentence(trialname, TANGO):
  ...
  j = 0
  for line in open(datasetfolder + correct_data, 'r'):
        itemList = line[:-1].split(',')
...
def Name_of_Place(THETA,datasetname):
    datasetNUM = 0
    # = "" #datasets[int(datasetNUM)]
    CN = []    #正解の音素列を格納する配列
    #correct_nameを読み込む
    for line in open(datasetfolder + correct_name, 'r'):
        itemList = line[:-1].split(',')
...
def Evaluation(filename,datasetname):
  #相互推定の学習結果データを読み込む
  ...
  TANGO = ReadTANGO(filename)
  CS_p = ReadCorrectSentence(filename, TANGO)
...
if __name__ == '__main__':
    ...
    Evaluation(filename,datasetname)
