...
data_name = raw_input("data_name?(**_NUM) > ")
data_num1 = "001" #raw_input("data_start_num?(DATA***) > ")
data_num2 = "004" #raw_input("data_end_num?(DATA***) > ")
...
LAR_M = [[] for c in xrange(N)]
MM = [ np.array([[] for m in xrange(10) ]) for n in xrange(N)]
#MM_M = 

fp = open('./data/Evaluation/' + data_name + '_' + data_num1 + '_' + data_num2 + '_Evaluation2.csv', 'w')
...
for s in range(N):
  i = 0
  for line in open('./data/' + data_name + '_' + str(S+s).zfill(2) + '/'+ data_name + '_' + str(S+s).zfill(2) + '_A_sougo_Evaluation2.csv', 'r'):
    ...
    if (i != 0) and (itemList[0] != '') and (i <= 10):
      ...
      LAR_M[s] = LAR_M[s] + [float(itemList[0])]
  ...
  if (LAR_M[s][-1] > LAR_MAX[0][1]):
          LAR_MAX = [[s+1,LAR_M[s][-1]]] + LAR_MAX
          
#print "MI_MAX:",MI_MAX
#print "ARI_MAX:",ARI_MAX
#print "PARw_MAX:",PARw_MAX
#print "PARs_MAX:",PARs_MAX
print "LAR_MAX:",LAR_MAX
#print MI_M
#MM_M = sum(MM)/N
#MI_MM = sum(MI_M)/N
#ARI_MM = sum(ARI_M)/N
...
for iteration in xrange(len(LAR_MM)):
  fp.write( str(LAR_MM[iteration]) ) #str(MI_MM[iteration])+','+ str(ARI_MM[iteration])+','+ str(PARs_MM[iteration])+','+str(PARw_MM[iteration]) )
  fp.write('\n')
fp.write('\n')
...
for iteration in xrange(len(LAR_MM)):
  ...
  LAR_MS = np.array([LAR_M[s][iteration] for s in xrange(N)])
  #print iteration,np.std(MI_MS, ddof=1)
  fp.write( str(np.std(LAR_MS, ddof=1)) ) #str(np.std(MI_MS, ddof=1))+','+ str(np.std(ARI_MS, ddof=1))+','+ str(np.std(PARs_MS, ddof=1))+','+str(np.std(PARw_MS, ddof=1)) )
  fp.write('\n')
#np.std
#float(ARI_M / N)
#print "ARI mean"
