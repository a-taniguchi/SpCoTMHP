...
import os.path
from Julius1best_decSpCoSLAM import *
#import time
    ...
    for xdata in xrange(len(Xp)):
        ...
        for c in xrange(L) :
          if(ctc[c] != 0):
            ...
            temp_bunbo = 0.0
            for c2 in range(L):
              W_bunbo_temp =  Multinomial(W[c2])
              bunbo_temp = W_bunbo_temp.pmf(Otb_B)
              temp_bunbo += bunbo_temp*pi[c2] #sum([W[c2][otb]*pi[c2] for c2 in range(L)])
            ##場所概念の多項分布、piの計算
            temp[c] = temp[c] * pi[c] / temp_bunbo
            ##場所概念の多項分布、piの計算
            #temp[c] = temp[c] * pi[c]
...
def Evaluation2(filename):
  ...
  fp = open('./data/' + filename + '/' + filename + '_A_sougo_EvaluationPRR_MI.csv', 'w')  
  ...
  fp_LAR = open('./data/' + filename + '/' + filename + '_A_sougo_PRR_MI.csv', 'w')  
