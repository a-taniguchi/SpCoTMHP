...
###########################################################
# Path-Planning Program by A star algorithm (ver. database) with costmap and emission probability
# Akira Taniguchi 2019/06/24-2019/07/02-2019/09/11
# Spacial Thanks: Ryo Ozaki
###########################################################

##Command: 
#python ./Astar_Database_weight.py trialname mapname iteration sample init_position_num speech_num
#python ./Astar_Database_weight.py 3LDK_01 s1DK_01 1 0 0 0
...
outputname = outputfile + "Astar_weight_Database_"+"N"+str(N_best)+"A"+str(Approx)+"S"+str(start)+"G"+str(speech_num)
...
while open_list:
    ...
    for act_func, act_cost in zip(action_functions, cost_of_actions):
        ...
        if (int(maze[q]) != 0):
            continue
        q_cost = p_cost + act_cost - np.log(PathWeightMap[q[0]][q[1]]) #+ (costmap[q[1]][q[0]]/100.0) #current sum cost and action cost
