...
outputname = outputfile + "Astar_Approx_"+"N"+str(N_best)+"A"+str(Approx)+"S"+str(start)+"G"+str(speech_num)
...
cost_of_actions  = np.log([    1/5,    1/5,  1/5,    1/5,    1/5]) #, ,    1,        1,        1,          1]
...
for gc_index in range(J):
    ...
    while open_list:
        ...
        for act_func, act_cost in zip(action_functions, cost_of_actions):
            ...
            q_pev = Manhattan_distance(q, goal) * np.log(1/5) #heuristic function
            q_key = q_cost - q_pev
    ...
    p_cost_candidate[gc_index] = p_cost
