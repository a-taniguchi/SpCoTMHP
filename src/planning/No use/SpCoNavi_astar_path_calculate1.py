...
def PostProbMap_nparray_jit(CostMapProb,Mu,Sig,map_length,map_width,it,phi_l): #,IndexMap):
        ...
        for i in range(K):
          PostP=multivariate_normal(Mu[it],Sig[it]).pdf(pos)
          if i==0:
        	PostProb=PostP*phi_l[i][it]
          else	:
               PostProb=PostP*phi_l[i][it] 
        '''
...
def SavePath(X_init, X_goal, Path, Path_ROS, outputname):
    ...
    #np.savetxt("start_" + str(start) + "goal_" + str(goal) + "_Path_ROS.csv", Path_ROS, delimiter=",")
...
start=(150,130)
Gl=4
outputname = outputfile + "Astar_SpCoNavi_"+"S"+str(start)+"_G"+str(Gl)
PathWeightMap = PostProbMap_nparray_jit(CostMapProb,Mu,Sig,map_length,map_width,Gl,phi_l)
#####描画
...
#ss=Map_coordinates_To_Array_index(Mu[St])


gg=Map_coordinates_To_Array_index(Mu[Gl])
print(gg[0])
