...
##Command: 
#python3 run_exp2_spcotmhp_astar_d.py trialname 
#python3 run_exp2_spcotmhp_astar_d.py 3LDK_01
...
print("RUN EXP2: SpCoTMHP (Baselines) A*")
        ...
        for tyukan in range(K):
            if (tyukan != i) and (tyukan != j): # 中間がスタートとゴールと同じにならないようにする
                ...
                if (speech_num != tyukan_num): #中間の単語インデックスとゴールの単語インデックスが同じにならないようにする
                    ...
                    process_call = "python3 spcotmhp_astar_execute_d.py " + trialname + " 1 0 " + str(init_position_num)  + " " + str(speech_num) + " " + str(st[0]) + " " +  str(st[1]) + " " + str(tyukan_num)
                    print(process_call)
                    #subprocess.Popen("pwd")
                    subprocess.run( process_call, shell=True ) #, stdout=PIPE, stderr=PIPE

                    process_call = "python3 spcotmhp_astar_execute_d2.py " + trialname + " 1 0 " + str(init_position_num)  + " " + str(speech_num) + " " + str(st[0]) + " " +  str(st[1]) + " " + str(tyukan_num)
                    print(process_call)
                    #subprocess.Popen("pwd")
                    subprocess.run( process_call, shell=True ) #, stdout=PIPE, stderr=PIPE


print("END EXP2: SpCoTMHP (Baselines) A*")
