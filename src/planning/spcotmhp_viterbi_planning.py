...
if __name__ == '__main__': 
    ...
    for i in range(K):
      for j in range(K):
        if i==j:
         ...
         fname=outputfile +"SpCoTMHPS"+str(i)+"G"+str(j)+"_Log_likelihood_sum.csv"
    ...
    for i in range(Step-1):
      if root[i]!=root[i+1]:
        fname=outputfile +"SpCoTMHPS"+str(root[i])+"G"+str(root[i+1])+"_Distance.csv"
        dis[i]=readdis(fname)
        dissum+=dis[i]  
    #print(dissum)
    print(dis)
    ...
    for v in range(len(root)-1):
    	...
    	i=0
    	if root[v]!=root[v+1]:
    	 for line in open(filename + "/navi/SpCoTMHPS" + str(root[v]) + 'G' + str(root[v+1]) +  '_Path.csv', 'r'):
    ...
    np.savetxt(outputname + "fin_distance.csv", disp, delimiter=",")
