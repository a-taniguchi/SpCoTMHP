...
##Command: 
#python3 run_exp2_spconavi_astar.py trialname 
#python3 run_exp2_spconavi_astar.py 3LDK_01
...
print("RUN EXP2: SpCoNavi Viterbi A*")
        ...
        for tyukan in range(K):
            if (tyukan != i) and (tyukan != j): # 中間がスタートとゴールと同じにならないようにする
                ...
                if (speech_num != tyukan_num): #中間の単語インデックスとゴールの単語インデックスが同じにならないようにする
                    ...
                    process_call = "python3 spconavi_astar_execute.py " + trialname + " s" + trialname + " 1 0 " + str(init_position_num)  + " " + str(speech_num) + " " + str(st[0]) + " " +  str(st[1]) + " " + str(tyukan_num)
                    print(process_call)

                    #if (process_call == "python3 spconavi_astar_execute.py 3LDK_06 s3LDK_06 1 0 -1 2 96 50 3"):
                    #    flag = 1
...
print("END EXP2: SpCoNavi Viterbi A*")
