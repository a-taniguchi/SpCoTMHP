...
##Command: 
#python3 spcotmhp_astar_multicand_metric.py trialname mapname iteration sample type_gauss
#python3 spcotmhp_astar_multicand_metric.py 3LDK_01 s3LDK_01 1 0 g
...
def Sampling_waypoint(THETA, it):
    #THETAを展開
    ...
    goal_candidate = multivariate_normal.rvs(mean=Mu[it], cov=S[it], size=N_ie-1, random_state=None)
    #for it in range(K):
    #count_it = 0
    #while (count_it < sample_it[0][it]):
    goal_candidate = [ tools.Map_coordinates_To_Array_index(goal_candidate[n]) for n in range(N_ie-1) ] + [ tools.Map_coordinates_To_Array_index(Mu[it]) ]
    #count_it += 1

    #Xt_max = Map_coordinates_To_Array_index( [ Xp[pox.index(max(pox))][0], Xp[pox.index(max(pox))][1] ] ) #[0.0,0.0] ##確率最大の座標候補
    goal_candidate_tuple = [(goal_candidate[j][1], goal_candidate[j][0]) for j in range(N_ie)]
    print("Waypoint candidates:", goal_candidate_tuple)
...
if (type_gauss == "g"):
    outputsubfolder = outputfile + "astar_node_gauss_multi/"
else:
...
waypoints = [Sampling_waypoint(THETA, it) for it in range(K)] #[K,N,(x,y)]

#すべてのwaypoint座標をファイル保存
output = outputsubfolder + "_waypoints.csv"
np.savetxt( output, np.array(waypoints), delimiter=",")
print("Save waypoints: " + output)
...
for st_i in range(K):
 for gl_i in range(K):
  if st_i == gl_i:
    ...
    #すでに計算済みの平均同士の パスを読み込む


    #for ni,nj in iteratools.product(waypoints,waypoints): #
    for n_st in range(N_ie): # for start index
        for n_gl in range(N_ie): # for goal index
            outputname = outputsubfolder + "Astar_SpCoTMHP_"+"S"+str(St)+"_G"+str(Gl)+"_Ns"+str(n_st)+"_Ng"+str(n_gl)
            
            if (n_st == N_ie-1) and (n_gl == N_ie-1): #平均点同士の場合
                print(n_st,n_gl)
            else:
                print(n_st,n_gl)
                ...
                # スタート：ガウスからサンプリングした中継点候補
                ss=tools.Map_coordinates_To_Array_index(waypoints[St][n_st])
                start=(ss[1],ss[0]) #スタート位置を指定

                # ゴール：ガウスからサンプリングした中継点候補
                gg=tools.Map_coordinates_To_Array_index(waypoints[Gl][n_gl])
