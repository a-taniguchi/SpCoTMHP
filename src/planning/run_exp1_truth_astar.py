...
for i,j in itertools.product(range(K), range(K)):
    #print(i,j)
    if (i != j): #(int(CoonectMatricx[i][j]) == 0): #直接の接続関係がないとき
        ...
        process_call = "python3 truth_astar_execute.py " + trialname + " s" + trialname + " 1 0 " + str(init_position_num)  + " " + str(j) + " " + str(st[0]) + " " +  str(st[1])
        print(process_call)
