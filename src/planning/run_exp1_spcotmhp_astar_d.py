...
##Command: 
#python3 run_exp1_spcotmhp_astar_d.py trialname 
#python3 run_exp1_spcotmhp_astar_d.py 3LDK_01
...
print("RUN EXP1: SpCoTMHP (Baselines) A*")
...
for i,j in itertools.product(range(K), range(K)):
    ...
    if (int(CoonectMatricx[i][j]) == 0): #直接の接続関係がないとき
        ...
        process_call = "python3 spcotmhp_astar_execute_d.py " + trialname + " 1 0 " + str(init_position_num)  + " " + str(speech_num) + " " + str(st[0]) + " " +  str(st[1]) + " -1"
        print(process_call)
        #subprocess.Popen("pwd")
        subprocess.run( process_call, shell=True ) #, stdout=PIPE, stderr=PIPE

        process_call = "python3 spcotmhp_astar_execute_d2.py " + trialname + " 1 0 " + str(init_position_num)  + " " + str(speech_num) + " " + str(st[0]) + " " +  str(st[1]) + " -1"
        print(process_call)
        #subprocess.Popen("pwd")
        subprocess.run( process_call, shell=True ) #, stdout=PIPE, stderr=PIPE
...
print("END EXP1: SpCoTMHP (Baselines) A*")
