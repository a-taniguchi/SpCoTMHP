...
##Command: 
#python3 evaluate2.py trialname method_id
#python3 evaluate2.py 3LDK_01 1
...
### 比較手法 "spconavi_astar_min_J1",
Methods = [ "spconavi_astar_min", "astar_result_d_gauss", "astar_result_d2_gauss", "dijkstra_result_wd", "dijkstra_result" ] 
...
for method in range(len(Methods)):
    ...
    SPL         = np.array([ [ [np.nan for i in range(K)] for c in range(K) ] for d in range(K) ])
    ...
    for i in reversed(range(K)):
      for j in reversed(range(K)):
        ...
        if (int(CoonectMatricx[i][j]) == 0): #直接の接続関係がないとき
            ...
            np.savetxt(output+"all_SPL.csv", SPL[i][j], delimiter=",", fmt='%s')
