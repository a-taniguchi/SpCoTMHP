...
# Path-Planning Program by A star algorithm (for SPL evaluation)
...
##Command: 
#python3 truth_astar_execute.py trialname mapname iteration sample init_position_num speech_num initial_position_x initial_position_y
#python3 truth_astar_execute.py 3LDK_01 s3LDK_01 1 0 0 7 100 100 
...
def Sampling_goal(Otb_B, THETA):
  ...
  for it in range(K):
      ...
      while (count_it < sample_it[0][it]):
        goal_candidate += [tools.Map_coordinates_To_Array_index(multivariate_normal.rvs(mean=Mu[it], cov=S[it], size=1, random_state=None))]
        count_it += 1
...
def a_star(start, goal, maze, action_functions, cost_of_actions, CostMapProb):
    ...
    while open_list:
        ...
        for act_func, act_cost in zip(action_functions, cost_of_actions):
            ...
            if (int(maze[q]) != 0):
                continue
            q_cost = p_cost - act_cost - np.log(CostMapProb[q[0]][q[1]])  #current sum cost and action cost 
...
outputsubfolder = outputfile + "truth_astar/"
outputname = outputsubfolder + "S"+str(start)+"G"+str(goal_ie)+"/"
...
CostMapProb = read_data.ReadCostMapProb(outputfile) #Ito
...
###goalの候補を複数個用意する
Mu_ai =  tools.Map_coordinates_To_Array_index(Mu[goal_ie]) 
goal_candidate = [ ( Mu_ai[1], Mu_ai[0] ) ] #[ Mu[EstimateGoal(Otb_B, THETA)] ]  #Sampling_goal(Otb_B, THETA) #(0,0)
J = 1 #Sampling_J #len(goal_candidate)
if(J != THETA[6]):
