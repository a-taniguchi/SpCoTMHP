...
##Command: 
#python3 run_exp1_spcotmhp_dijkstra.py trialname 
#python3 run_exp1_spcotmhp_dijkstra.py 3LDK_01
...
print("RUN EXP1: SpCoTMHP (Dijkstra)")
...
for i,j in itertools.product(range(K), range(K)):
    ...
    if (int(CoonectMatricx[i][j]) == 0): #直接の接続関係がないとき
        ...
        process_call = "python3 spcotmhp_dijkstra_execute.py " + trialname + " 1 0 " + str(init_position_num)  + " " + str(speech_num) + " " + str(st[0]) + " " +  str(st[1]) + " -1"
        print(process_call)
...
print("END EXP1: SpCoTMHP (Dijkstra)")
