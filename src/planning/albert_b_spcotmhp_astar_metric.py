...
###########################################################
# SpCoTMHPi: Spatial Concept-based Path-Planning Program for albert-b
...
##Command: 
#python3 albert_b_spcotmhp_astar_metric.py trialname mapname iteration sample type_gauss
#python3 albert_b_spcotmhp_astar_metric.py albertL2R01 map 1 2 g
#python3 albert_b_spcotmhp_astar_metric.py albertTMHP01 map 1 6 g
...
#重みはガウスか正規化ガウスか
type_gauss  = "g" #sys.argv[5] # g: gauss, ng: normalized gauss
...
for st_i in range(K):
 for gl_i in range(K):
  if st_i == gl_i:
  ...
  elif (psi[st_i][gl_i] >= ignore_value) and (list(It).count(st_i) > 1) and (list(It).count(gl_i) > 1):
    St=st_i
    Gl=gl_i
    ...
    save_data.SaveProbMap_TMHP( PathWeightMap, outputsubfolder, St ,Gl)
    #print("shape",gridmap.shape,PathWeightMap.shape)
    ...
    # スタート：ガウス平均
    ss=tools.Map_coordinates_To_Array_index_albert(Mu[St])
    start=(ss[1],ss[0]) #スタート位置を指定

    # ゴール：ガウス平均
    gg=tools.Map_coordinates_To_Array_index_albert(Mu[Gl])
    goal_candidate = [[gg[1],gg[0]]]
    print("st,gl",St,Gl,start,goal_candidate[0])
